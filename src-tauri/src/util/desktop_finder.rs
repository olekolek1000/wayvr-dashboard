...
fn search_internal(path: &str) -> anyhow::Result<Vec<DesktopEntry>> {
		...
		.filter(|e| e.file_type().is_file())
	{
		...
		let ini = match Ini::load_from_file(&file_path) {
			...
			Err(e) => {
				log::error!(
					"Failed to read INI for .desktop file {}: {:?}",
					file_path,
					e
				);
				continue;
...
fn search(path: &str) -> Vec<DesktopEntry> {
	match search_internal(path) {
		Ok(res) => res,
		Err(e) => {
...
pub fn find_entries() -> anyhow::Result<Vec<DesktopEntry>> {
	...
	entries.append(&mut search("/var/lib/flatpak/exports/share/applications/"));
